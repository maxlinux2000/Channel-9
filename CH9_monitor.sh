            ...
            if [ $size2 -lt "$DURATION" ]; then
                ...
                if [ ! -z $DTMF ]; then
                    ...
                    if [ $ENABLE = 1 ]; then
                        ...
                        # 2. Buscar palabras clave (KEYWORDS contiene 'palabra1|palabra2|...')
                        if echo "$TRANSCRIPT" | grep -E -q -i "$KEYWORDS"; then
                            
                            echo "🚨 ALERTA DETECTADA: Palabra clave encontrada: $KEYWORDS"
...
"
                            ...
                            echo "INFO: Modo Monitor CB: No se detectaron palabras clave ($KEYWORDS). Omitiendo alerta."
                        fi
                    fi
                fi
            fi
